...
minibatch_size=32  # This is how you set the minibatch size.  Note: if
...
while [ $x -lt $num_iters ]; do
    ...
    $cmd $gpu_cmd_opt $dir/log/train.$x.$j.log \
         nnet3-chain-train2  \
            ...
            --out-of-range-regularize=$out_of_range_regularize \
             ...
             ${model_out_prefix}.$j.raw || touch $dir/.error_diagnostic &
  done
  wait
  if [ -f $dir/.error ]; then
  ...
  if [ $x -ge 1 -a $num_jobs -gt 1 ]; then
  ...
  elif [ $x -ge 1 -a $num_jobs -eq 1 ]; then
      mv ${model_out_prefix}.1.raw $dir/$next_x.raw
  else
